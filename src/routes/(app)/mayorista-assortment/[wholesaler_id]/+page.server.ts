...
export const load: PageServerLoad = async ({ locals, params, url }) => {
  ...
  let products: AssortmentProductRow[] = (productsResult.data ?? []).map((product) => ({
    ...product,
    ...
    tableMessage:
      products.length > 0
        ...
        : {
            ...
            detail: 'No encontramos productos con los filtros aplicados para este mayorista.',
          },
  };
};
...
export const actions: Actions = {
  toggleProduct: async ({ request, locals }) => {
    ...
    if (existingResult.data?.id) {
      ...
      const { error } = await locals.supabase.from('wholesaler_products').insert({
