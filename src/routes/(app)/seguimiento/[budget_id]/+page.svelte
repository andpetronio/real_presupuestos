<script lang="ts">
  import { Badge, Button, Card, Input, Label, Select, Table, TableBody, TableBodyCell, TableBodyRow, TableHead, TableHeadCell, Textarea } from 'flowbite-svelte';
...
<section class="grid grid-cols-1 gap-4 xl:grid-cols-12">
  <Card size="xl" class="p-6 shadow-sm xl:col-span-8">
  ...
  <Card size="xl" class="p-6 shadow-sm xl:col-span-4">
    <p class="text-sm font-semibold text-gray-900">Registrar cobro parcial</p>
  ...
  <Card size="xl" class="p-6 shadow-sm xl:col-span-12">
    <p class="text-sm font-semibold text-gray-900">Progreso por receta</p>
              ...
              <div class="rounded-md bg-gray-50 p-3">
                ...
                <div class="mt-2 grid grid-cols-1 gap-2 md:grid-cols-2">
                  <div>
                    ...
                    <p class="mt-1 text-xs text-gray-500">Resta preparar: {recipe.pendingPreparationDays} días</p>
                  </div>
                  ...
                  <div>
                    ...
                    <p class="mt-1 text-xs text-gray-500">Resta entregar: {recipe.pendingDeliveryDays} días</p>
                  </div>
                </div>
              </div>
            {/each}
  ...
  <Card size="xl" class="p-6 shadow-sm xl:col-span-6">
    ...
    <form method="POST" action="?/addPreparation" class="mt-3 space-y-3">
      ...
      <div>
        ...
        <Textarea id="preparationNotes" name="notes" rows={2} />
      </div>
  ...
  <Card size="xl" class="p-6 shadow-sm xl:col-span-6">
    ...
    <form method="POST" action="?/addDelivery" class="mt-3 space-y-3">
      ...
      <div>
        ...
        <Textarea id="deliveryNotes" name="notes" rows={2} />
      </div>
  ...
  <Card size="xl" class="p-6 shadow-sm xl:col-span-6">
    <p class="text-sm font-semibold text-gray-900">Preparaciones y entregas</p>

    <div class="mt-3 grid gap-3 md:grid-cols-2">
      <div>
        <p class="mb-2 text-xs uppercase tracking-wide text-gray-500">Preparaciones</p>
        <div class="space-y-2">
          {#if data.preparations.length === 0}
            <p class="text-sm text-gray-500">Sin preparaciones.</p>
          {:else}
            {#each data.preparations as preparation (preparation.id)}
              <div class="rounded border border-gray-200 p-2 text-xs">
                <p class="font-medium">{preparation.recipe_days} días · {formatDate(preparation.prepared_at)}</p>
      ...
      <div>
        <p class="mb-2 text-xs uppercase tracking-wide text-gray-500">Entregas</p>
        <div class="space-y-2">
          {#if data.deliveries.length === 0}
            <p class="text-sm text-gray-500">Sin entregas.</p>
          {:else}
            {#each data.deliveries as delivery (delivery.id)}
              <div class="rounded border border-gray-200 p-2 text-xs">
                <p class="font-medium">{delivery.recipe_days} días · {formatDate(delivery.delivered_at)}</p>
