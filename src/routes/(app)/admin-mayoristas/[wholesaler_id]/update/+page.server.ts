...
export const load: PageServerLoad = async ({ locals, params }) => {
  const wholesalerResult = await locals.supabase
    .from('wholesalers')
    .select('id, name, unique_random_code, min_total_units, is_active, notes, category_id, tax_id, contact_full_name, contact_whatsapp, contact_email, address, delivery_preference, payment_preference')
...
export const actions: Actions = {
  ...
  regenerateCode: async ({ locals, params }) => {
    ...
    if (error) {
      return fail(400, { operatorError: 'No pudimos actualizar el código del mayorista.' });
    }
