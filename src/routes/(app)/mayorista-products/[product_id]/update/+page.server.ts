...
import { deleteWholesaleProductImage, extractImageFiles, parsePrice, parseText, uploadWholesaleProductImages } from '$lib/server/wholesale-backoffice/products';
...
export const actions: Actions = {
  ...
  uploadImages: async ({ request, locals, params }) => {
    ...
    if (files.length === 0) {
      ...
      console.log('[DEBUG] valores del campo images:', formData.getAll('images'));
  ...
  deleteImage: async ({ request, locals }) => {
    ...
    const result = await deleteWholesaleProductImage({ supabase: locals.supabase, imageId });
    if (!result.ok) {
