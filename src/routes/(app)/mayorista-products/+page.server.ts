...
import { applyListFilters, buildFallbackError, getPagination, getTableState } from '$lib/server/shared/list-helpers';
...
export const load: PageServerLoad = async ({ locals, url }) => {
  ...
  try {
    let query = locals.supabase
      ...
      .select('id, name, presentation, price_ars, description, is_active, created_at', { count: 'exact' })
      .order('created_at', { ascending: false });

    if (filters.search) {
      query = query.or(`name.ilike.%${filters.search}%,presentation.ilike.%${filters.search}%`);
    }
    ...
    const imagesResult = productIds.length > 0
    ...
    const imagesByProductId = new Map<string, Array<{ id: string; public_url: string; sort_order: number }>>();
...
export const actions: Actions = {
  toggleActive: async ({ request, locals }) => {
    ...
    if (error) {
      return fail(400, { operatorError: 'No pudimos actualizar el estado del producto.' });
    }

    return { operatorSuccess: shouldActivate ? 'Producto restaurado.' : 'Producto desactivado.' };
  },
};
