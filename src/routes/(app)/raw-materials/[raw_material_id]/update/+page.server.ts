...
export const load: PageServerLoad = async ({ locals, params }) => {
  ...
  const { data, error } = await locals.supabase
    .from('raw_materials')
    .select('id, name, base_unit, purchase_quantity, base_cost, wastage_percentage')
    .eq('id', rawMaterialId)
    .single();
...
export const actions: Actions = {
  update: async ({ request, locals, params }) => {
    ...
    if (!rawMaterialId || !name || !baseUnit || purchaseQuantity === null || baseCost === null || wastagePercentage === null) {
      return fail(400, {
        operatorError:
    ...
    if (error) {
      return fail(400, {
        operatorError: 'No pudimos guardar los cambios de la materia prima. Reintentá en unos segundos.',
        values: {
          name,
          baseUnit,
          purchaseQuantity: purchaseQuantityRaw,
