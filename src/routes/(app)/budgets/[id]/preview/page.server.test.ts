  ...
  it('genera enlace de WhatsApp Web válido con emojis preservados', async () => {
    ...
    const from = vi.fn((table: string) => {
            ...
            eq: vi.fn().mockReturnValue({
              single: vi.fn().mockResolvedValue({
                data: {
                  ...
                  tutor: { full_name: 'Ana Tutor', whatsapp_number: '+54 9 11 1234 5678' }
                },
                error: null
              })
            })
      ...
      if (table === 'settings') {
        return {
          select: vi.fn().mockReturnValue({
            eq: vi.fn().mockReturnValue({
              single: vi.fn().mockResolvedValue({
                data: { whatsapp_default_template: 'Hola {{tutor_nombre}} 😊🐶 para {{perros}}. Link {{link_presupuesto}}' },
                error: null
              })
            })
          })
      ...
      if (table === 'budget_dogs') {
        return {
          select: vi.fn().mockReturnValue({
            eq: vi.fn().mockResolvedValue({ data: [{ dog: { name: 'Nanuk' } }], error: null })
          })
        };
      }
    ...
    } as unknown as Parameters<(typeof actions)['sendWhatsapp']>[0])) as { waUrl?: string; error?: string };

    expect(result.error).toBeUndefined();
    expect(result.waUrl).toContain('https://web.whatsapp.com/send?phone=5491112345678&text=');

    const textParam = result.waUrl ? new URL(result.waUrl).searchParams.get('text') ?? '' : '';
    ...
    expect(decodedMessage).toContain('https://test.local/budget-response/token123');
    ...
    const from = vi.fn((table: string) => {
      if (table === 'budgets') {
        return {
          select: vi.fn().mockReturnValue({
            eq: vi.fn().mockReturnValue({
              single: vi.fn().mockResolvedValue({ data: { id: 'b-1', status: 'draft' }, error: null })
            })
          }),
...
describe('(app)/budgets/[id]/preview markSent', () => {
  it('devuelve error si el presupuesto no existe', async () => {
    ...
    const result = (await actions.markSent({
      params: { id: 'non-existent' },
      locals: { supabase: { from } }
    } as unknown as Parameters<(typeof actions)['markSent']>[0])) as { error?: string };
    ...
    const from = vi.fn((table: string) => {
      if (table === 'budgets') {
        return {
          select: vi.fn().mockReturnValue({
            eq: vi.fn().mockReturnValue({
              single: vi.fn().mockResolvedValue({ data: { id: 'b-1', status: 'sent' }, error: null })
            })
          })
        };
      }
