...
import {
  sendWhatsappAction,
  markSentAction,
} from "$lib/server/budgets/preview/actions";
...
export const load: PageServerLoad = async ({ params, locals }) => {
  ...
  try {
    ...
    const [{ data: budgetBase, error: budgetError }, settingsResult] =
      await Promise.all([
        locals.supabase
          .from("budgets")
          .select(
        ...
        locals.supabase
          .from("settings")
          .select(
            "vacuum_bag_small_unit_cost, vacuum_bag_large_unit_cost, label_unit_cost, non_woven_bag_unit_cost, labor_hour_cost, cooking_hour_cost, calcium_unit_cost, kefir_unit_cost, meal_plan_margin",
          )
          .eq("id", 1)
          .single(),
      ]);
    ...
    if (budgetError || !budgetBase) {
      return {
        ...
        pageMessage: {
          ...
          detail: "El presupuesto que buscás no existe.",
          actionLabel: "Volver",
        },
      };
    }
    ...
    const tutorName =
      (budgetBase.tutor as { full_name?: string } | null)?.full_name ??
      "Sin tutor";
    const budget: BudgetPreviewData = {
      ...budgetBase,
    ...
    const dogIdByBudgetDogId = new Map(
      budgetDogs.map((bd) => [bd.id, bd.dog_id]),
    );
    ...
    const recipeIds = Array.from(
      new Set((budgetDogRecipesResult.data ?? []).map((bdr) => bdr.recipe_id)),
    );
    ...
    const dogNameById = new Map(
      (dogsResult.data ?? []).map((d) => [d.id, d.name]),
    );
    const recipeNameById = new Map(
      (recipesResult.data ?? []).map((r) => [r.id, r.name]),
    );
    ...
    const rawMaterialIds = Array.from(
      new Set(recipeItems.map((ri) => ri.raw_material_id)),
    );
    ...
    const materialCostById = new Map(
      (rawMaterialsResult.data ?? []).map((rm) => [
        rm.id,
        {
          name: rm.name,
    ...
    const assignments: DogRecipeAssignment[] = (
      budgetDogRecipesResult.data ?? []
    ).map((bdr) => ({
      dogId: dogIdByBudgetDogId.get(bdr.budget_dog_id) ?? "",
      dogName:
        dogNameById.get(dogIdByBudgetDogId.get(bdr.budget_dog_id) ?? "") ?? "",
    ...
    for (const assignment of assignments) {
      const itemsForRecipe = recipeItems.filter(
        (ri) => ri.recipe_id === assignment.recipeId,
      );
    ...
    const operationalItems: OperationalLineItem[] = [
      {
        name: "Bolsas vacío(chicas)",
        quantity: budget.vacuum_bag_small_qty,
        unitCost: settings.vacuum_bag_small_unit_cost,
        subtotal:
          budget.vacuum_bag_small_qty * settings.vacuum_bag_small_unit_cost,
      },
      {
        name: "Bolsas vacío(grandes)",
        quantity: budget.vacuum_bag_large_qty,
        unitCost: settings.vacuum_bag_large_unit_cost,
        subtotal:
          budget.vacuum_bag_large_qty * settings.vacuum_bag_large_unit_cost,
      },
      {
        name: "Etiquetas",
        quantity: budget.labels_qty,
        unitCost: settings.label_unit_cost,
        subtotal: budget.labels_qty * settings.label_unit_cost,
      },
      {
        name: "Bolsas de fiselina",
        quantity: budget.non_woven_bag_qty,
        unitCost: settings.non_woven_bag_unit_cost,
        subtotal: budget.non_woven_bag_qty * settings.non_woven_bag_unit_cost,
      },
      {
        name: "Horas mano de obra",
        quantity: budget.labor_hours_qty,
        unitCost: settings.labor_hour_cost,
        subtotal: budget.labor_hours_qty * settings.labor_hour_cost,
      },
      {
        name: "Horas cocción",
        quantity: budget.cooking_hours_qty,
        unitCost: settings.cooking_hour_cost,
        subtotal: budget.cooking_hours_qty * settings.cooking_hour_cost,
      },
      {
        name: "Calcio",
        quantity: budget.calcium_qty,
        unitCost: settings.calcium_unit_cost,
        subtotal: budget.calcium_qty * settings.calcium_unit_cost,
      },
      {
        name: "Kefir",
        quantity: budget.kefir_qty,
        unitCost: settings.kefir_unit_cost,
        subtotal: budget.kefir_qty * settings.kefir_unit_cost,
      },
