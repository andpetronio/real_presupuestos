...
export const load: PageServerLoad = async ({ locals, params }) => {
  ...
  const [recipeResult, recipeItemsResult, dogsResult, rawMaterialsResult] = await Promise.all([
    locals.supabase
      .from('recipes')
...
export const actions: Actions = {
  update: async ({ request, locals, params }) => {
    ...
    const { error: deleteItemsError } = await locals.supabase.from('recipe_items').delete().eq('recipe_id', recipeId);
    if (deleteItemsError) {
      return fail(400, {
    ...
    const { error: insertItemsError } = await locals.supabase.from('recipe_items').insert(recipeItemsPayload);
    if (insertItemsError) {
      return fail(400, {
