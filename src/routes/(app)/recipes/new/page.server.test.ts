...
describe('(app)/recipes/new actions.create', () => {
  ...
  it('crea receta con items y redirige', async () => {
    ...
    const from = vi.fn((table: string) => {
      ...
      if (table === 'recipes') {
        return { insert: recipeInsert, delete: vi.fn().mockReturnValue({ eq: vi.fn() }) };
      }
    ...
    expect(recipeItemsInsert).toHaveBeenCalledWith([
      { recipe_id: 'r-1', raw_material_id: 'm-1', daily_quantity: 100, unit: 'g' },
      { recipe_id: 'r-1', raw_material_id: 'm-2', daily_quantity: 50, unit: 'g' }
    ]);
  });
});
