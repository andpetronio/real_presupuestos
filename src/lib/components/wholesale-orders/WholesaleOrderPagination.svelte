<script lang="ts">
  ...
  type Props = { page: number; totalPages: number; total: number; search: string; status: string };
...
{#if totalPages > 1}
  <nav class="mt-4 flex items-center justify-between" aria-label="Paginación de pedidos mayoristas">
    <div>
      {#if hasPrev}
        <Button href={buildPaginationHref('/mayorista-orders', page - 1, { search, status })} size="sm" color="light">Anterior</Button>
      {:else}
    ...
    <div>
      {#if hasNext}
        <Button href={buildPaginationHref('/mayorista-orders', page + 1, { search, status })} size="sm" color="light">Siguiente</Button>
      {:else}
