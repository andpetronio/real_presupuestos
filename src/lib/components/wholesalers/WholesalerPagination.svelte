...
{#if totalPages > 1}
  <nav class="mt-4 flex items-center justify-between" aria-label="Paginación de mayoristas">
    <div>
      {#if hasPrev}
        <Button href={buildPaginationHref('/admin-mayoristas', page - 1, { search, status, sortBy, sortDir })} size="sm" color="light">Anterior</Button>
      {:else}
    ...
    <p class="text-sm text-gray-600" aria-live="polite">
      ...
      &nbsp;·&nbsp;
      <span>{total} mayorista{total !== 1 ? 's' : ''}</span>
    </p>

    <div>
      {#if hasNext}
        <Button href={buildPaginationHref('/admin-mayoristas', page + 1, { search, status, sortBy, sortDir })} size="sm" color="light">Siguiente</Button>
      {:else}
