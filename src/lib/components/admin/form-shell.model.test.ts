...
describe("form-shell.model", () => {
  it("resuelve mensajes por estado de formulario", () => {
    ...
    expect(resolveFormShellMessage("error")).toBe(
      "No se pudo guardar. Reintentá y verificá los datos.",
    );
    expect(resolveFormShellMessage("success")).toBe(
      "Cambios guardados correctamente.",
    );
  });

  it("prioriza mensaje explícito por encima del estado", () => {
    expect(resolveFormShellMessage("error", "Mensaje custom")).toBe(
      "Mensaje custom",
    );
  });
  ...
  it("resuelve label de acción primaria según estado", () => {
    expect(resolvePrimaryLabel("idle", "Guardar cambios")).toBe(
      "Guardar cambios",
    );
