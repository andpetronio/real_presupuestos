...
{#if totalPages > 1}
  <nav
    ...
    aria-label="Paginación de perros"
  >
    <!-- Previous -->
    <div>
      {#if hasPrev}
        <Button
          href={buildPaginationHref('/dogs', page - 1, { search, status, sortBy, sortDir })}
          size="sm"
          color="light"
    ...
    <p class="text-sm text-gray-600" aria-live="polite">
      ...
      &nbsp;·&nbsp;
      <span>{total} perro{total !== 1 ? 's' : ''}</span>
    </p>
