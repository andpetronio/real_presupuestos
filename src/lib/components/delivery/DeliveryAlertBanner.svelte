    ...
    <div class="flex flex-wrap items-center gap-3">
      <div class="flex-1 min-w-0">
        ...
        {#if overdueAlerts.length > 0}
          <p class="mt-2 text-xs font-semibold uppercase tracking-wide text-red-700">Entregas con demora</p>
        ...
        {#if dueSoonAlerts.length > 0}
          <p class="mt-3 text-xs font-semibold uppercase tracking-wide text-accent-700">Entregas próximas</p>
          <ul class="mt-1 space-y-1">
            {#each dueSoonAlerts as alert (alert.budgetDogRecipeId + '-' + alert.dayOfMonth)}
              {@const urgency = alert.daysOffset <= 1 ? 'text-orange-700 font-medium' : 'text-gray-600'}
