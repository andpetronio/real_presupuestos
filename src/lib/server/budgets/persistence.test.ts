        ...
        select: vi.fn().mockReturnValue({
          eq: vi.fn().mockReturnValue({
            maybeSingle: vi.fn().mockResolvedValue({
              data: { id: 'b-1', status: 'draft', tutor_id: 't-1', final_sale_price: 1000, expires_at: null, public_token: 'tok-1', reference_month: '2026-01', reference_days: 20 },
              error: null
            })
          })
        }),
...
describe('saveBudgetComposition', () => {
  ...
  it('happy path — deletes, inserts budget_dogs, inserts budget_dog_recipes → ok', async () => {
    ...
    expect(insertBudgetDogRecipes).toHaveBeenCalledWith(
      expect.arrayContaining([
        expect.objectContaining({ budget_dog_id: 'bd-1', recipe_id: 'r-1', assigned_days: 10 })
      ])
    );
  });
  ...
  it('error path — insert budget_dogs fails → returns error', async () => {
    ...
    expect(result.ok).toBe(false);
    if (!result.ok) expect(result.message).toContain('No pudimos guardar los perros');
  });
  ...
  it('error path — insert budget_dog_recipes fails → returns error', async () => {
    ...
    (mockSupabase.from as ReturnType<typeof vi.fn>).mockImplementation((table: string) => {
      ...
      if (table === 'budget_dog_recipes') {
        return { insert: vi.fn().mockReturnValue({ error: { message: 'recipe insert failed' } }) };
      }
      return {};
    });
    ...
    expect(result.ok).toBe(false);
    if (!result.ok) expect(result.message).toContain('No pudimos guardar las recetas');
  });
  ...
  it('validation path — dogTotals includes dog not in composition mapping → returns error', async () => {
    ...
    const orphanDogTotals = [{ dogId: 'd-orphan', ingredientTotal: 100, operationalTotal: 0, totalCost: 100, finalSalePrice: 125 }];
...
describe('saveBudgetSnapshot', () => {
  it('happy path — insert succeeds → ok', async () => {
    ...
    const result = await saveBudgetSnapshot({
      ...
      snapshotData: {
        tutorId: 't-1',
        composition: [],
        operationals: { vacuumBagSmallQty: 0, vacuumBagLargeQty: 0, labelsQty: 0, nonWovenBagQty: 0, laborHoursQty: 0, cookingHoursQty: 0, calciumQty: 0, kefirQty: 0 },
        settings: {},
        totals: {}
      }
    });
  ...
  it('error path — insert fails → returns error with specific message', async () => {
    const insert = vi.fn().mockReturnValue({ error: { message: 'snapshot failed' } });
    const mockSupabase = makeMockSupabase();
    (mockSupabase.from as ReturnType<typeof vi.fn>).mockImplementation((table: string) => {
      if (table === 'budget_snapshots') return { insert };
      return {};
...
describe('updateBudgetStatus', () => {
  ...
  it('error path — update fails → returns error with specific message', async () => {
    ...
    expect(result.ok).toBe(false);
    if (!result.ok) expect(result.message).toBe('No pudimos actualizar el presupuesto.');
  });
...
describe('deleteBudget', () => {
  it('happy path — delete succeeds → ok', async () => {
    ...
    const result = await deleteBudget({ budgetId: 'b-1', supabase: mockSupabase });
...
describe('getBudgetById', () => {
  it('found — returns budget data', async () => {
    ...
    const result = await getBudgetById({ budgetId: 'b-1', supabase: mockSupabase });
  ...
  it('not found / error — returns null', async () => {
    ...
    const result = await getBudgetById({ budgetId: 'b-nonexistent', supabase: mockSupabase });

    expect(result).toBeNull();
  });

  it('error from supabase — returns null', async () => {
    const maybeSingle = vi.fn().mockResolvedValue({ data: null, error: { message: 'server error' } });
    const eq = vi.fn().mockReturnValue({ maybeSingle });
    const select = vi.fn().mockReturnValue({ eq });
    const mockSupabase = makeMockSupabase();
    (mockSupabase.from as ReturnType<typeof vi.fn>).mockImplementation((table: string) => {
