...
describe("buildDogsSummary", () => {
  ...
  it("1 perro → días + nombre", () => {
    expect(buildDogsSummary([{ name: "Mora", requestedDays: 10 }])).toBe(
      "10 días para Mora",
    );
  });
...
const makeWhatsappSupabase = (
            ...
            overrides.settingsResult ?? {
              data: {
                ...
                whatsapp_default_template:
                  "Hola {{tutor}}, tu presupuesto: {{total_final}}",
  ...
  return {
    from: vi.fn((table: string) => base[table as keyof typeof base] ?? {}),
...
describe("sendBudgetWhatsapp", () => {
  describe("happy path", () => {
    ...
    it("usa web.whatsapp.com en desktop user agent", async () => {
      ...
      expect(result).toMatchObject({ ok: true });
      if ("ok" in result && result.ok && "waMeUrl" in result) {
        expect(result.waMeUrl).toContain(
  ...
  describe("error paths", () => {
    it("budget no encontrado → error", async () => {
      ...
      expect(result).toMatchObject({
        ok: false,
        message: "No encontramos el presupuesto a enviar.",
      });
    });
    ...
    it("tutor no encontrado → error", async () => {
      ...
      expect(result).toMatchObject({
        ok: false,
        message: "No encontramos el tutor del presupuesto.",
      });
    });
    ...
    it("tutor sin número de WhatsApp → error", async () => {
      const supabase = makeWhatsappSupabase({
        tutorResult: {
          data: { full_name: "Ana García", whatsapp_number: null },
          error: null,
        },
      });
      const result = await sendBudgetWhatsapp({
    ...
    it("tutor con número vacío → error", async () => {
      const supabase = makeWhatsappSupabase({
        tutorResult: {
          data: { full_name: "Ana García", whatsapp_number: "" },
          error: null,
        },
      });
      const result = await sendBudgetWhatsapp({
    ...
    it("template vacío → error", async () => {
      ...
      expect(result).toMatchObject({
        ok: false,
        message: expect.stringContaining("mensaje base configurado"),
      });
    });
    ...
    it("template con Unicode roto → error", async () => {
      const supabase = makeWhatsappSupabase({
        settingsResult: {
          data: {
            business_name: "Test",
            whatsapp_default_template: "Hola \ufffd mundo",
            whatsapp_signature: "",
          },
          error: null,
        },
    ...
    it("signature con Unicode roto → error", async () => {
      ...
      expect(result).toMatchObject({
        ok: false,
        message: expect.stringContaining("firma"),
      });
    });
    ...
    it("funciona con un solo perro", async () => {
      const supabase = makeWhatsappSupabase({
        budgetDogsResult: {
          data: [{ requested_days: 30, dog: { name: "Mora" } }],
          error: null,
        },
      });
      const result = await sendBudgetWhatsapp({
