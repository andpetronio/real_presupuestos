...
export const calculateBudgetTotals = (input: {
  ...
  const recipeDailyCostMap = new Map(
    input.recipeDailyCosts.map((entry) => [entry.recipeId, entry.dailyCost]),
  );
  ...
  for (const assignment of input.assignments) {
    ...
    ingredientByDog.set(
      assignment.dogId,
      (ingredientByDog.get(assignment.dogId) ?? 0) + subtotal,
    );
  }

  const ingredientTotal = roundMoney(
    Array.from(ingredientByDog.values()).reduce((acc, value) => acc + value, 0),
  );

  // Delegate to the shared pure helper — no more duplication.
  const operationalTotal = computeOperationalTotal(
    input.operationals,
    input.settings,
  );
  ...
  const dogs = Array.from(ingredientByDog.entries()).map(
    ([dogId, ingredient]) => ({
      dogId,
  ...
  const ingredientTotalForSplit = dogs.reduce(
    (acc, dog) => acc + dog.ingredient,
    0,
  );

  const dogTotals: CalculatedDogTotals[] = dogs.map((dog) => {
    const ratio =
      ingredientTotalForSplit > 0
        ? dog.ingredient / ingredientTotalForSplit
        : dogs.length > 0
          ? 1 / dogs.length
          : 0;
