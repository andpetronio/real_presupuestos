...
export const buildWhatsappContext = (params: {
  ...
  const { tutorFullName, dogsSummary, finalSalePrice, expiresAt, referenceMonth, referenceDays, businessName, link, whatsappNumber } = params;

  const now = new Date();
  const referenceDate = referenceMonth ? new Date(`${referenceMonth}T00:00:00`) : now;
  const referenceMonthLabel = Number.isNaN(referenceDate.getTime()) ? getMonthLabel(now) : getMonthLabel(referenceDate);
...
export const sendBudgetWhatsapp = async (params: {
  ...
  if (budgetTyped.status !== 'draft' && budgetTyped.status !== 'ready_to_send') {
    return { ok: false, message: 'Solo podés enviar presupuestos en borrador o listos para enviar.' };
  }
  ...
  if (!toNumber) {
    return { ok: false, message: 'El tutor no tiene un número de WhatsApp válido para envío.' };
  }
  ...
  if (settingsError || !settings) {
    return { ok: false, message: 'No pudimos leer la configuración de WhatsApp para enviar el presupuesto.' };
  }
  ...
  if (!template) {
    return {
      ok: false,
      message: 'No hay un mensaje base configurado para WhatsApp. Completalo en Configuración > WhatsApp / Mensajería y reintentá.'
    };
  }
  ...
  if (containsBrokenUnicode(template)) {
    return {
      ok: false,
      message: 'El mensaje base tiene caracteres inválidos (�). Guardalo de nuevo en Configuración escribiendo o pegando los emojis correctos.'
    };
  }
  ...
  if (containsBrokenUnicode(rendered)) {
    return {
      ok: false,
      message: 'El mensaje generado para WhatsApp contiene caracteres inválidos (�). Revisá nombres, plantilla y variables antes de enviar.'
    };
  }

  // 6. Agregar firma
  const normalizedSignature = settings.whatsapp_signature ? normalizeTemplateText(settings.whatsapp_signature) : '';
  if (containsBrokenUnicode(normalizedSignature)) {
    return {
      ok: false,
      message: 'La firma de WhatsApp tiene caracteres inválidos (�). Guardala de nuevo en Configuración pegando los emojis correctos.'
    };
  }
  ...
  if (containsBrokenUnicode(finalMessage)) {
    return {
      ok: false,
      message: 'El mensaje final de WhatsApp contiene caracteres inválidos (�). Revisá plantilla, firma y datos del tutor.'
    };
  }
