...
describe("toFormValuesFromSettings", () => {
  ...
  it("round-trip: form → save → load preserva valores", () => {
    ...
    expect(result.payload.whatsapp_default_template).toBe(
      "Hola {{tutor_nombre}}",
    );
    ...
    expect(result.payload.satisfaction_survey_enabled).toBe(true);
    expect(result.payload.satisfaction_survey_url).toBe(
      "https://example.com/survey",
    );
    expect(result.payload.satisfaction_survey_message).toBe(
