...
export const containsBrokenUnicode = (text: string): boolean =>
  text.includes("\uFFFD");

export const normalizeTemplateText = (text: string): string =>
  text.normalize("NFC");
...
export const renderWhatsappTemplate = (
  ...
  const renderedDoubleBraces = template.replace(
    doubleBracesPlaceholderRegex,
    resolveValue,
  );
  return renderedDoubleBraces.replace(
    singleBracesPlaceholderRegex,
    resolveValue,
  );
};
...
const mobileWhatsappUserAgentRegex =
...
export const buildWhatsappSendUrl = (params: {
  ...
  const encodedMessage = encodeURIComponent(
    normalizeTemplateText(params.message),
  );
  const isMobile = Boolean(
    params.userAgent && mobileWhatsappUserAgentRegex.test(params.userAgent),
  );
