...
describe("validateRecipeItems", () => {
  ...
  it("retorna error si falta rawMaterialId", () => {
    const result = validateRecipeItems([
      { rawMaterialId: "", dailyQuantity: "100" },
    ]);
    expect(result.ok).toBe(false);
  });
  ...
  it("retorna error si falta dailyQuantity", () => {
    const result = validateRecipeItems([
      { rawMaterialId: "id1", dailyQuantity: "" },
    ]);
    expect(result.ok).toBe(false);
  });
  ...
  it("retorna error si dailyQuantity no es positiva", () => {
    const result = validateRecipeItems([
      { rawMaterialId: "id1", dailyQuantity: "0" },
    ]);
    expect(result.ok).toBe(false);
...
describe("getTutorError", () => {
  ...
  it("mensaje específico para WhatsApp duplicado", () => {
    expect(
      getTutorError("create", "ERROR: violate tutors_whatsapp_number_unique"),
    ).toContain("WhatsApp");
  });
});
...
describe("getVeterinaryError", () => {
  ...
  it("mensaje específico para nombre duplicado", () => {
    expect(
      getVeterinaryError("create", "ERROR: violate veterinaries_name_unique"),
    ).toContain("nombre");
  });
});
...
describe("getRawMaterialError", () => {
  ...
  it("mensaje específico para nombre duplicado", () => {
    expect(
      getRawMaterialError("create", "ERROR: raw_materials_name_unique"),
    ).toContain("nombre");
  });
});
...
describe("getRecipeError", () => {
  ...
  it("mensaje específico para foreign key error", () => {
    expect(
      getRecipeError("create", "violates foreign key constraint"),
    ).toContain("perro");
  });
});
...
describe("getRecipeItemsError", () => {
  ...
  it("mensaje específico para material duplicado", () => {
    expect(
      getRecipeItemsError("ERROR: recipe_items_unique_material_per_recipe"),
    ).toContain("repitas");
  });
});
