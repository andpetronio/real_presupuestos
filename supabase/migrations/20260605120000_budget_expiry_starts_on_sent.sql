...
declare
  v_token text;
  v_respondable_statuses public.budget_status[] := array[
    'sent'::public.budget_status
  ];
  v_expirable_statuses public.budget_status[] := array[
    'sent'::public.budget_status
  ];
  v_budget record;
begin
  ...
  if
    v_budget.status = any(v_expirable_statuses)
  ...
  then
    ...
    where id = v_budget.id
      and status = any(v_expirable_statuses);
  ...
  select
    ...
    v_budget.tutor_name,
    (
      v_budget.status = any(v_respondable_statuses)
...
begin
  ...
  if
    v_status = any(v_expirable_statuses)
  ...
  if v_status <> all(v_respondable_statuses) then
    return query
    select
      false,
      case
  ...
  where id = v_budget_id
    and status = any(v_respondable_statuses);
